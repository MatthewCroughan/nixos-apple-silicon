{ config, pkgs, lib, ... }:
{
  ...
  in mkIf config.hardware.asahi.enable {
    ...
    systemd.services.extract-asahi-firmware = mkIf (!cfg.usePeripheralFirmwareFromNixStore) {
      description = "Extract Asahi Firmware";
      ...
      script = ''
        ...
        ${pkgs.mount}/bin/mount /dev/disk/by-partuuid/`cat /proc/device-tree/chosen/asahi,efi-system-partition` /tmp/.fwsetup/esp
        ${pkgs'.asahi-fwextract}/bin/asahi-fwextract /tmp/.fwsetup/esp/asahi /tmp/.fwsetup/extracted
        ${pkgs.umount}/bin/umount /tmp/.fwsetup/esp

        pushd /tmp/.fwsetup/
        cat /tmp/.fwsetup/extracted/firmware.cpio | ${pkgs.cpio}/bin/cpio -id --quiet --no-absolute-filenames
        mkdir -p /lib/firmware
        cp -ru vendorfw/* /lib/firmware
        popd
        rm -rf /tmp/.fwsetup
      '';
    };
